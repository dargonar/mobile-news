...
- (NSString *)gtm_stringByEscapingHTMLUsingTable:(HTMLEscapeMap*)table 
  ...
  if (!buffer) {
    ...
    if (!data) {
      ...
      //HACK: _GTMDevLog(@"couldn't alloc buffer");
      return nil;
      // COV_NF_END
    }
  ...
  if (!buffer || !data2) {
    // COV_NF_START
    //HACK: _GTMDevLog(@"Unable to allocate buffer or data2");
    return nil;
    // COV_NF_END
  }
  ...
  for (NSUInteger i = 0; i < length; ++i) {
    ...
    if (val || (escapeUnicode && buffer[i] > 127)) {
      ...
      if (val) {
        ...
        //HACK: _GTMDevAssert(escapeUnicode && buffer[i] > 127, @"Illegal Character");
