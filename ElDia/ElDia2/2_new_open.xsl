...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <html>
      <head>
        ...
        <link rel="stylesheet" type="text/css" media="only screen and (max-device-width: 480px)" href="2.css" />
        <title>XXX</title>
      </head>
      <body bgcolor="#ffffff" >
  ...
  <xsl:template name="GalleryTemplate">
    <xsl:variable name="gallery">
      <xsl:for-each select="rss/channel/item/media:group/media:content">
        <xsl:value-of select="concat( substring('; ','{@url}'),.)"/>
      </xsl:for-each>
    </xsl:variable>
    <!-- xsl:value-of select="$gallery=concat($gallery, ';', {@url})"/ -->
  </xsl:template>
  
  <xsl:template match="rss/channel/item">
    <!-- xsl:call-template name="GalleryTemplate" / -->
    <section class="entry_open">
      <div>
        <section class="header">
          ...
          <a href="gallery://{$gallery}" title="galeria">
        ...
        <xsl:choose>
          <xsl:when test="not(not(media:thumbnail))">
            <xsl:call-template name="MainImageTemplate">
              <xsl:with-param name="ImageUrl" select="media:thumbnail/@url"/>
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
            <xsl:if test="not(not(media:group/media:content))" >
              <xsl:apply-templates name="ImageUrl" select="media:group/media:content[1]/@url"/>
            </xsl:if>
          </xsl:otherwise>
        </xsl:choose>
        
        <!-- xsl:if test="not(not(media:thumbnail))" >
  ...
  <xsl:template name="MainImageTemplate">
